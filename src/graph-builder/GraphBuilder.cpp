    ...
    bool VisitStmt(Stmt *s){
        ...
        for(auto parent : parents){
            if(parent.get<Stmt>()){
                ...
                std::cout<<"I'm not sure what this is: " <<parent.getNodeKind().asStringRef().str()<<std::endl;
            }
        }
        ...
        if(isa<DeclRefExpr>(s)){
            ...
            if(d->getDecl()->getNameAsString() == "__VERIFIER_nondet_int"){
                std::cout<<"(AST,("<<s<<","<<s->getStmtClassName()<<")"<<","<<"("<<d->getDecl()<<","<<"input_int"<<"))"<<std::endl;
            }
            else{
        ...
        for(auto parent : parents){
            if(parent.get<Stmt>()){
                ...
                if(isa<FunctionDecl>(d)){
                    ...
                    if(f->getNameInfo().getAsString()=="__VERIFIER_nondet_int"){
                        std::cout <<"(AST,("<<dPrime<<","<<dPrime->getDeclKindName()<<")"<<","<<"("<<d<<","<<"input"<<"))"<<std::endl;    
                    }
                    else{
...
public:
    ...
    void printCFGPair(const Stmt* s1, const Stmt* s2){
        std::cout<<"(CFG,("<<s1<<","<<s1->getStmtClassName()<<"),("<<s2<<","<<s2->getStmtClassName()<<"))"<<std::endl;
    }
    
    void printCFGPair(const Stmt* s, const Decl* d){
        std::cout<<"(CFG,("<<s<<","<<s->getStmtClassName()<<"),("<<d<<","<<d->getDeclKindName()<<"))"<<std::endl;
    }
    ...
    bool VisitCallExpr(CallExpr *call){
        ...
        if(successor.get<Stmt>()){ 
            std::cout<<"(CFG,("<<funDecl<<",FunctionExit),("<<successor.get<Stmt>()<<","<<successor.get<Stmt>()->getStmtClassName()<<")"<<std::endl;
        }
        else if(successor.get<FunctionDecl>()){
            std::cout<<"(CFG,("<<funDecl<<",FunctionExit),("<<successor.get<FunctionDecl>()<<",FunctionExit))"<<std::endl;
        }
        else if(successor.get<Decl>()){
            std::cout<<"(CFG,("<<funDecl<<",FunctionExit),("<<successor.get<Decl>()<<","<<successor.get<Decl>()->getDeclKindName()<<"))"<<std::endl;
        }
    ...
    bool VisitIfStmt(IfStmt *i){
        ...
        if(thenStmt){
            ...
            if(successor.get<Stmt>()){ 
                ...
                std::cout<<"(CFG,("<<finalNode<<","<<finalNode->getStmtClassName()<<"),("<<successor.get<FunctionDecl>()<<",FunctionExit))"<<std::endl;
            }
            else if(successor.get<Decl>()){
        ...
        if(elseStmt){
            ...
            if(successor.get<Stmt>()){
                ...
                std::cout<<"(CFG,("<<i<<","<<i->getStmtClassName()<<"),("<<successor.get<FunctionDecl>()<<",FunctionExit))"<<std::endl;
            }
            else if(successor.get<Decl>()){
    ...
    bool VisitReturnStmt(ReturnStmt *r){
        ...
        if(parent->get<FunctionDecl>()){
            std::cout<<"(CFG,("<<r<<","<<r->getStmtClassName()<<"),("<<parent->get<FunctionDecl>()<<",FunctionExit))"<<std::endl;
        }
    ...
    bool VisitWhileStmt(WhileStmt *w){
        ...
        if(successor.get<Stmt>()){ 
            ...
            std::cout<<"(CFG,("<<w<<","<<w->getStmtClassName()<<"),("<<successor.get<FunctionDecl>()<<",FunctionExit))"<<std::endl;
        }
        else if(successor.get<Decl>()){
    ...
    bool VisitDoStmt(DoStmt *d){
        ...
        if(successor.get<Stmt>()){ 
            ...
            std::cout<<"(CFG,("<<d<<","<<d->getStmtClassName()<<"),("<<successor.get<FunctionDecl>()<<",FunctionExit))"<<std::endl;
        }
        else if(successor.get<Decl>()){
    ...
    bool VisitForStmt(ForStmt *f){
        ...
        if(successor.get<Stmt>()){ 
            ...
            std::cout<<"(CFG,("<<f<<","<<f->getStmtClassName()<<"),("<<successor.get<FunctionDecl>()<<",FunctionExit))"<<std::endl;
        }
        else if(successor.get<Decl>()){
    ...
    bool VisitBreakStmt(BreakStmt *b){
        ...
        if(parent->get<Stmt>()){
            ...
            if(successor.get<Stmt>()){ 
                ...
                std::cout<<"(CFG,("<<b<<","<<b->getStmtClassName()<<"),("<<successor.get<FunctionDecl>()<<",FunctionExit))"<<std::endl;
            }
            else if(successor.get<Decl>()){
    ...
    bool VisitDecl(Decl *d){
        if(isa<FunctionDecl>(d)){
            ...
            if(f->hasBody()){
                ...
                std::cout<<"(CFG,("<<f<<","<<stringRep<<"),("<<functionHead<<","<<functionHead->getStmtClassName()<<"))"<<std::endl;
            }
        }
        return true;
    }
...
public:
    explicit GraphBuilderConsumer(ASTContext *Context) : VisitorAST(Context), VisitorCFG(Context) {}
