...
class GGNN(nn.Module):
    ...
    def collect_incoming(self, node_spot, nodes, edges, incoming):
        """
    ...
    def forward(self, problemClass, nodesBatch, backwards_edge_dictBatch,incoming):
        """
        ...
        for _ in range(self.passes):   
            ...
            for i in range(len(nodesBatch)):
                ...
                for node_spot in range(len(nodesBatch[i])):
                    inputs.append(self.collect_incoming(node_spot, nodesBatchCopy[i], edge_dict, incoming[i]))
...
def my_collate(batch):
    ...
    problemClass = [item[0][1].cuda() for item in batch]
...
def train_model(model, loss_fn, batchSize, trainset, valset, optimizer, num_epochs):
    ...
    for epoch in range(0, num_epochs):
        ...
        for (i, ((tokenSets, problemTypes, backwards_edge_dicts), labels)) in enumerate(tqdm.tqdm(train_loader)):
            incoming = torch.zeros(len(labels),150).cuda()
            for item in range(len(labels)):
                tokenSets[item] = tokenSets[item].cuda()
                labels[item] = labels[item].cuda()
            problemTypes.cuda()

            scores = model(problemTypes, tokenSets, backwards_edge_dicts, incoming)
