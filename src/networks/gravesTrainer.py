...
import torch, json, time, argparse, os
...
if __name__ == '__main__':
	...
	testFiles = json.load(open("../../data/cpa21Results.json"))
	testLabels = [(key, [item[1] for item in testFiles[key]]) for key in testFiles if os.path.exists("../../data/final_graphs21/"+key.split("|||")[0]+".npz")]
	...
	returnString = "graves_"+str(args).replace("\'","").replace(",","").strip("Namespace").strip("(").strip(")").replace(" ","_") + "_" + str(int(time.time()))
